...
const baseInjections = [
  ...
  {
    label: "schedule midline epidural steroid injection at",
    levels: [],
    direction: false
  },
  { label: "schedule midline caudal block at", levels: [], direction: false },
  {
    label: "schedule TFESI at",
    levels: [],
    direction: true,
    directionAfter: true
  },
  {
    label: "schedule hip injection (intra-articular) at",
    levels: [],
    direction: true,
    directionAfter: true
  },
  {
    label: "schedule trochanteric bursa hip injection at",
    levels: [],
    direction: true,
    directionAfter: true
  },
  {
    label: "schedule knee injection (intra-articular) at",
    levels: [],
    direction: true,
    directionAfter: true
  },
  {
    label: "schedule subacromial shoulder injection at",
    levels: [],
    direction: true,
    directionAfter: true
  },
  {
    label: "schedule shoulder injection (intra-articular) at",
    levels: [],
    direction: true,
    directionAfter: true
  },
  ...
  {
    label: "schedule SCS implantation lumbar at",
    levels: [],
    direction: false
  },
  {
    label: "schedule trigger point injection at",
    levels: [],
    direction: false
  },
...
const InjectionsList = ({ onInjectionChange }) => {
  ...
  const handleChange = (index, field, value) => {
    ...
    else updated[index][field] = field === "notes" ? value.trim() : value || "";
  ...
  //   const lines = includedInjections.map((inj, idx) => {
  //     const levelText = inj.selectedLevel || inj.levels?.join(", ") || "";
  //     const direction = inj.directionSelected || "";
  //     const timing = inj.timing ? `${inj.timing} schedule` : "";
  //     return `${idx + 1}. ${timing} ${inj.label} ${levelText} ${direction}`.trim();
  //   });

  //   const formattedSummary = lines.length > 0 ? `INJECTIONS:\n${lines.join("\n")}` : "";

  //   if (onInjectionChange) {
  //     onInjectionChange({
  //       injections: includedInjections,
  //       INJECTION_SUMMARY: formattedSummary
  //     });
  //   }
...
//   useEffect(() => {
//   const includedInjections = injections.filter((inj) => inj.included);
//   const lines = includedInjections.map((inj) => {
//     const parts = [
...
//   const summary = lines.length > 0 ? `INJECTIONS:\n${lines.join("\n")}` : "";
...
useEffect(() => {
  ...
  const heading = "Injections:"; // for docx styling if recognized
  const summary = lines.length > 0 ? `${heading}\n  ${lines.join("\n  ")}` : "";
    ...
    <div style={styles.container}>
      ...
      {injections
        ...
        .map((inj, idx) => (
          <div
            key={idx}
            style={{ ...styles.injectionRow, background: "#f9fafb" }}
          >
        ...
        <div key={index} style={styles.injectionRow}>
          ...
          {inj.direction && !inj.directionAfter && (
            <select
              ...
              onChange={(e) =>
                handleChange(index, "directionSelected", e.target.value)
              }
              style={styles.select}
            >
          ...
          {inj.label === "schedule" ? (
            <input
              ...
              onChange={(e) =>
                handleChange(index, "selectedLevel", e.target.value)
              }
